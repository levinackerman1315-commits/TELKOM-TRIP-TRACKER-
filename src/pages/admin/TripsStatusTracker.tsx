  ...
  //                         {new Date(
  //                           history.find(h => h.status === step.status)!.created_at
          ...
          <div className="flex items-center justify-center gap-3 p-6 bg-red-50 border border-red-200 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-red-700">
                {history.length > 0 && history[history.length - 1]?.notes
                  ? history[history.length - 1].notes
                  : 'Settlement has been rejected and returned to employee'}
              </p>
              <p className="text-xs text-red-600 mt-2 font-medium">
                → Employee can upload new receipts and submit for review again
              </p>
            </div>
          </div>
        ) : (
