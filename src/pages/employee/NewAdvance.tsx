...
// import { Textarea } from "@/components/ui/textarea"
// import { Alert, AlertDescription } from "@/components/ui/alert"
// import { ArrowLeft, DollarSign, FileText, Info, CheckCircle2, AlertCircle, Loader2, MapPin, Calendar, Lock, Unlock, Upload, X } from 'lucide-react'
// import { advanceAPI, tripAPI } from '@/services/api'
// import { Trip, Advance } from '@/types'
...
//       submitData.append('trip_id', String(tripId))
//       submitData.append('request_type', formData.request_type)
//       submitData.append('requested_amount', formData.requested_amount)
//       submitData.append('request_reason', formData.request_reason)
...
//         💡 Tip: Upload supporting document below to strengthen your request
//       </p>
//     </AlertDescription>
//   </Alert>
// )}
...
//                   Supporting Document <span className="text-muted-foreground text-xs">(Optional)</span>
//                 </Label>
...
//                       </div>
//                       <div>
//                         <p className="text-sm font-medium">{supportingDocument.name}</p>
//                         <p className="text-xs text-muted-foreground">
...
//                 <p className="text-xs text-muted-foreground">
//                   Upload supporting documents like quotations, estimates, or other relevant files
//                 </p>
//               </div>
//             </CardContent>
//             <CardFooter className="flex items-center justify-between bg-muted/50 border-t">
...
export default function NewAdvance() {
  ...
  const fetchExistingAdvances = async () => {
    try {
      ...
      // Auto-select berdasarkan kondisi
      const hasApprovedInitial = advances.some(adv => 
        adv.request_type === 'initial' && 
        ['approved_area', 'approved_regional', 'completed'].includes(adv.status)
      )
  ...
  const validate = () => {
    ...
    // ✅ CRITICAL: Validate lock status
    if (formData.request_type === 'initial' && isInitialLocked) {
      er.submit = 'Cannot submit: Initial advance already exists or is pending'
      setErrors(er)
      return false
    }
    
    if (formData.request_type === 'additional' && isAdditionalLocked) {
      if (!hasApprovedInitial) {
        er.submit = 'Cannot submit: Initial advance must be approved first'
      } else if (hasPendingAdditional) {
        er.submit = 'Cannot submit: You have a pending additional request. Please wait for approval or rejection.'
      }
      setErrors(er)
      return false
    }
        ...
        <form onSubmit={submit}>
          <Card className="shadow-soft mb-6">
            ...
            <CardContent className="space-y-6">
              ...
              {isAdditionalLocked && hasApprovedInitial && hasPendingAdditional && (
                <Alert className="bg-amber-50 border-amber-200">
                  <Info className="h-4 w-4 text-amber-600" />
                  <AlertDescription className="text-amber-800">
                    <p className="font-medium mb-1">Additional Request Pending</p>
                    <p className="text-sm">
                      You have an additional advance request that is currently being processed.
                      Please wait for approval or rejection before submitting a new request.
                    </p>
                  </AlertDescription>
                </Alert>
              )}
              ...
              {!isAdditionalLocked && hasApprovedInitial && (
                <Alert className="bg-green-50 border-green-200">
                  ...
                  <AlertDescription className="text-green-800">
                    ...
                    <p className="text-xs font-medium text-green-900 mt-2">
                      💡 Tip: Upload supporting document below to strengthen your request
                    </p>
                  </AlertDescription>
                </Alert>
            ...
            <CardFooter className="flex items-center justify-between bg-muted/50 border-t">
              ...
              <Button 
                ...
                disabled={
                  isLoading || 
                  successMessage !== '' ||
                  (formData.request_type === 'initial' && isInitialLocked) ||
                  (formData.request_type === 'additional' && isAdditionalLocked)
                }
              >
                {isLoading ? (
                  <>
