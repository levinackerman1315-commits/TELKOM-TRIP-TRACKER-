...
const UserManagement = () => {
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      // ✅ IMPROVED: Set validation error state instead of just showing dialog
      if (!userId || isNaN(userId) || userId <= 0) {
        console.error('Invalid user ID:', { id, userId })
        setValidationError(`Invalid user ID: "${id}". Redirecting to dashboard...`)
        setTimeout(() => navigate('/hr/dashboard'), 2000)
        return
      }
  ...
  // ✅ NEW: Early return for validation error
  if (validationError) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <AlertCircle className="h-16 w-16 text-red-600 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-red-600 mb-2">Invalid User ID</h2>
